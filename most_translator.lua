...
local function translator(input, segment, env)
    ...
    local candidates, upper_pattern, accent_pattern = latin2russian(env, input)
    ...
    for _, cand in ipairs(candidates) do
        yield(Candidate("completion", segment.start, segment._end, apply_pattern(env, cand, upper_pattern, accent_pattern), ""))
    end
end
